...
class GroundPickBlockNotOnShelfController(Geom2dRobotController):
    ...
    def _generate_waypoints(
        ...
        raise RuntimeError("Failed to find a collision-free path to target.")
...
class GraphPickBlockNotOnShelfSkill(BaseClutteredStorage2DSkill):
    ...
    def _generate_action_plan(
        ...
        """Generate action plan using hardcoded parameters."""
        # Hardcode parameters for consistent behavior
        grasp_ratio = 0.5  # Grasp from right side, middle height
        ...
        arm_length = (min_arm_length + max_arm_length) / 2  # Use medium extension

        # Create temporary controller with hardcoded params
...
class GraphPickBlockOnShelfSkill(BaseClutteredStorage2DSkill):
    ...
    def _generate_action_plan(
        ...
        """Generate action plan using hardcoded parameters."""
        # Hardcode parameters for consistent behavior
        grasp_ratio = 0.5  # Grasp from top/bottom, middle position
        robot_obj = state.get_objects(CRVRobotType)[0]
        max_arm_length = state.get(robot_obj, "arm_length")
        min_arm_length = (
            state.get(robot_obj, "base_radius")
...
class GraphPlaceBlockOnShelfSkill(BaseClutteredStorage2DSkill):
    ...
    def _generate_action_plan(
        ...
        """Generate action plan using hardcoded parameters."""
        # Hardcode parameters for consistent placement in shelf
        relative_dx = 0.5  # Center horizontally
        relative_dy = 0.5  # Center vertically (inside shelf)

        # Create temporary controller with hardcoded params
        controller = GroundPlaceBlockOnShelfController(
            objects, self._action_space, self._init_constant_state
        )
        controller._current_params = (relative_dx, relative_dy)
...
class GraphPlaceBlockNotOnShelfSkill(BaseClutteredStorage2DSkill):
    ...
    def _generate_action_plan(
        ...
        """Generate action plan using hardcoded parameters."""
        # Hardcode parameters for consistent placement away from shelf
        rel_x = 0.5  # Center in available x space
        rel_y = 0.5  # Center in available y space
        rel_theta = 0.5  # Neutral orientation
